...
class UserController extends Controller
{
  public function home()
  {
    ...
    $totalRef = $user->left + $user->right;
    $totalLeft = $user->left;
    $totalRight = $user->right;
    $totalBvCut = BvLog::where('user_id', $user->id)->where('trx_type', '-')->sum('amount');
    $totalIndirect = UserCommission::where('referral_id', $user->id)->sum('commission');
    $totalDirect = $user->total_ref_com - $totalIndirect;

    $totalBv = @$user->userExtra->bv_left + @$user->userExtra->bv_right;

    return view($this->activeTemplate . 'user.dashboard', compact('pageTitle', 'user', 'totalDeposit', 'submittedDeposit', 'pendingDeposit', 'rejectedDeposit', 'totalWithdraw', 'submittedWithdraw', 'pendingWithdraw', 'rejectWithdraw', 'totalRef', 'totalBvCut', 'totalLeft', 'totalRight', 'totalBv', 'totalIndirect', 'totalDirect'));
  }
  ...
  public function binaryTree(Request $request, $username = null)
  {
    ...
    // if ($username)
    // {
    //   $user = User::where('username', $username)->firtst();
    // }
    ...
    $tree = [];
    $tree[0] = $user;
    $treeService->buildTree($user, $tree, 1);
    $tree2 = $treeService->buildTree2($tree);


    return view($this->activeTemplate . 'user.tree', compact('pageTitle', 'tree2', 'tree', 'user'));
  }
