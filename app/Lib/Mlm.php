...
class Mlm
{
  ...
  {
    while (0 == 0) {
      $upper = User::where('id', $user->id)->first();
      if (!$upper) {
        break;
      }
  ...
  protected function getPositionUser($id, $position, $excludeIds = [])
{
    return User::where('ref_by', $id)
                ->where('position', $position)
                ->whereNotIn('id', $excludeIds)
                ->with('referrer', 'plan', 'userExtra')
                ->first();
}
 ...
 public function showTreePage($user, $isAdmin = false)
{
    ...
    $hands['a'] = $user;
    $hands['b'] = $this->getPositionUser($user->id, 1, $array);
    if ($hands['b']) {
        $array[] = $hands['b']->id;
        $hands['d'] = $this->getPositionUser($user->id, 1, $array);
        $hands['e'] = $this->getPositionUser($user->id, 2, $array);
    }
    if ($hands['d']) {
        $array[] = $hands['d']->id;
        $hands['h'] = $this->getPositionUser($user->id, 1, $array);
        $hands['i'] = $this->getPositionUser($user->id, 2, $array);
    }
    if ($hands['e']) {
        $array[] = $hands['e']->id;
        $hands['j'] = $this->getPositionUser($user->id, 1, $array);
        $hands['k'] = $this->getPositionUser($user->id, 2, $array);
    }
    $hands['c'] = $this->getPositionUser($user->id, 2, $array);
    if ($hands['c']) {
        $array[] = $hands['c']->id;
        $hands['f'] = $this->getPositionUser($user->id, 1, $array);
        $hands['g'] = $this->getPositionUser($user->id, 2, $array);
    }
    if ($hands['f']) {
        $array[] = $hands['f']->id;
        $hands['l'] = $this->getPositionUser($user->id, 1, $array);
        $hands['m'] = $this->getPositionUser($user->id, 2, $array);
    }
    if ($hands['g']) {
        $array[] = $hands['g']->id;
        $hands['n'] = $this->getPositionUser($user->id, 1, $array);
        $hands['o'] = $this->getPositionUser($user->id, 2, $array);
    }
    return $hands;
}
   ...
   * @return string
   */
  public function showSingleUserinTree($userArray, $isAdmin = false)
  {
    $html = '';

    $user = $userArray ? User::find($userArray['id']) : null;
    ...
    if ($user) {
      ...
      if(!$isAdmin){
        $username = $user->username == $authUser->username ? "You" : $user->username;
